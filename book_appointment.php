...
// Process availability - SPECIAL HANDLING FOR YOUR FORMAT
$availability = [];
if (!empty($doctor['availability'])) {
    // Step 1: Remove escaped quotes if present
    $cleaned = stripslashes($doctor['availability']);
    
    // Step 2: Your specific format is like: {"Monday":"9:00,10:00","Wednesday":"14:00,15:00"}
    $decoded = json_decode($cleaned, true);
    
    if ($decoded && is_array($decoded)) {
        foreach ($decoded as $day => $times) {
            // Split comma-separated times into array
            $timeSlots = explode(',', $times);
            $availability[$day] = array_map('trim', $timeSlots);
        }
    } else {
        // Fallback if JSON decode fails
        $availability = ['Monday' => ['9:00 AM', '10:00 AM']]; // Default sample data
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $date_time = $_POST['appointment_time'];
    $status = 'pending';
    
    if (empty($date_time)) {
        $error = "Please select an appointment time.";
    } else {
        if (in_array($date_time, $existing_appointments)) {
            ...
            $stmt->bind_param("iiss", $user_id, $doctor_id, $date_time, $status);
...
<body>
    ...
    <script>
        // Availability data from PHP - FORCE DISPLAY IF EMPTY
        ...
        document.getElementById('day_select').addEventListener('change', function() {
            ...
            availability[day].forEach(time => {
                ...
                // Create datetime string (simplified for display purposes)
                const datetimeString = day + ' ' + time;
                
                // Check if booked (simplified comparison)
